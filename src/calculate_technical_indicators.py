...
def calculate_technical_indicators(formatted_price_data, fast_window, slow_window, rsi_window, sentiment_summary):
    """
    Calculate technical indicators (slow SMA, fast SMA, EMA, and RSI) for each ticker to be used as features in a machine learning model.

    Args:
        formatted_price_data (DataFrame): DataFrame with ticker columns and date-indexed close prices.
        fast_window (int): The window length for fast moving averages.
        slow_window (int): The window length for slow moving averages.
    ...
    for ticker in data.columns:
        ...
        # Forward Fill (ffill) the sentiment score sum 
        data[f'Sentiment_Score_Sum_{ticker}'] = data[f'Sentiment_Score_Sum_{ticker}'].fillna(method='ffill')
        # Backward Fill (bfill) the sentiment score sum 
        data[f'Sentiment_Score_Sum_{ticker}'] = data[f'Sentiment_Score_Sum_{ticker}'].fillna(method='bfill')
